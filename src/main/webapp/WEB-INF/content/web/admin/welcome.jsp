...
<html>
	...
	function editPass() {
		if ($("#oldPassword").val().trim() == "") {
			tips("原始密码不能为空", "#oldPassword");
			$("#oldPassword").select();
			return;
		}
		if ($("#newPassword").val().trim() == "") {
			tips("新密码不能为空", "#newPassword");
			$("#newPassword").select();
			return;
		}
		if ($("#newPassword_c").val().trim() == "") {
			tips("确认新密码不能为空", "#newPassword_c");
			$("#newPassword_c").select();
			return;
		}
		if ($("#newPassword").val().trim() != $("#newPassword_c").val().trim()) {
			tips("两次新密码不一致", "#newPassword_c");
			$("#newPassword_c").select();
			return;
		}
	...
	<div class="mainindex">
		<div class="welinfo">
			<span><img src="${ctx}/images/sun.png" alt="天气" /></span> <b>${sessionServerUserInfo.name }(${sessionServerUserInfo.username })你好，欢迎使用TBMP管理系统</b>
		</div>
