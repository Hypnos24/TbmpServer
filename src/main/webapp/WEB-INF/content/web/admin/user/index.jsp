...
<html>
			...
			success : function(result) {
				if (!isOutTime(result)) {
					...
					if ($("#isAdmin").val().trim() == "true") {
						var roleIdArray = JSON.parse($("#roleIds").val().trim());
	...
	function resetPass(userId) {
		layer.confirm("确定重置该用户密码为系统初始化密码?", {
		    btn: ["确定","取消"], //按钮
		    shade: 0.1 //显示遮罩
		}, function(){
			msg("正在处理中...");// 为了让对话框马上消失
			$(".btn-link").button('loading');
			var url = "${ctx}/web/admin/user/reset_pass";
			$.ajax({
				url:url,
				type:'post',
				data:{
