...
<html>
	...
	function add(){
		 if($("#index_new").val().trim() ==""){
		 	tips("字典key不能为空", "#index_new");
		 	$("#index_new").select();
		 	return ;
		 }
		  if($("#value_new").val().trim() ==""){
		 	tips("字典值不能为空", "#value_new");
		 	$("#value_new").select();
		 	return ;
		 }
		$(".btn-link").button('loading');
	...
	<div class="rightinfo">
		<div class="usual">
			<div class="itab">
				<ul>
					<li><a href="#tab1" class="selected" onclick="setModule(1);">运营商</a></li>
				</ul>
			</div>
		</div>
		<ul class="seachform">
				<li>
					...
					<input name="index" id="index" type="text" class="span2" placeholder="字典KEY"/>
				</li>
				<li>
					<label>
						字典值:
					</label>
					<input name="value" id="value" type="text" class="span2" placeholder="字典值"/>
				</li>
				<li>
					<label>
						&nbsp;
